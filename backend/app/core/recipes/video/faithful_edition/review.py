...
REVIEW_INSTRUCTION = (
    ...
    "irrelevant/unclear frames; an empty selection is allowed, never force an illustration. "
    "Captions must be short plain text describing only visible evidence, not inferred prices, "
    "Introduce NO numeric values absent from the corresponding edited paragraph. For example, "
    "when the paragraph says support around 104, say the highlighted support region around 104, "
    "not a newly read decimal price. Describe unspoken chart periods/measurements without numbers. "
    "profitability or outcomes. Do not transcribe tiny numbers from unclear pixels. Clearly "
    "distinguish diagram arrows from actual future movements. Do not modify the body based on "
