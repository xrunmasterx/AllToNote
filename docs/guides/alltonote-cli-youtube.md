...
B 站源码下载器 `BilibiliDownloader.download_video` 默认选择当前登录态可访问的最佳视频流：
分辨率优先，其次帧率，其余沿用 [yt-dlp 质量排序](https://github.com/yt-dlp/yt-dlp#sorting-formats)。
不限制 MP4 或编码，有独立音轨时合并为 MKV，不转码；单流保留实际容器。
文件名包含所选格式 ID，每次先获取可用画质再复用相同格式文件，不直接复用旧的同名 MP4。
这不改变 CLI 的仅音频下载路径，也不新增分辨率参数；`--quality balanced` 仍不是视频画质设置。
...
→ 生成可选全文概览、章节关键点、配图正文和原始／校正／编辑对照。
图文模式优先按模型核验的主题边界分章，输入字节预算仍是硬上限；无图模式保持原来的分段策略。
每个正文段落最多映射 12 条连续片段，不混淆不同对象的案例或在半句话中断段。

- 保留原顺序、论证、例子、条件和有意义的重复；只排除明确的音乐／噪声标记。
- 编辑及独立复核按完整观点／步骤核对对象、动作、条件、数字单位、时间限制、确定程度、理由及例外，
  不使用视频类别分支或领域关键词决定重要性。连续的简短举例归入共同话题，独立展开讨论时再分章。
  同一段内无新增信息的重述可合并，但保留全部来源 ID；不同事件、实际重复执行的步骤和观点修正不能合并。
  数字与术语校验允许删除重复出现次数，但仍禁止丢掉独有值、增加出现次数或打乱数字顺序；
  这不证明语义等价，独立模型复核仍须检查每条观点及其关系。长度比例仅记录，不设压缩目标。
...
- `draft show --presentation reading` 隐藏系统引用脚注、编辑对照、局部“待核对”、辅助“待复核项”和内部时间标记，保留正文图片及章节时间标题。
  ...
  新稿只在开头概览呈现选中的主要结论，章节块只保留一组关键点，不再重复一段章节摘要。
  简短且正文已说明清楚的内容可省略整个块，不生成“无”占位；取舍由模型按内容判断，不使用字数阈值。
  摘要和关键点必须保留所选结论的重要条件、期限和限定。标签使用普通加粗，不进入目录；
  旧审计稿重新导出阅读版时仍兼容旧摘要标签，不自动重写已生成的内容。
